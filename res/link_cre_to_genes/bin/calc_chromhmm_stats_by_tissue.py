...
f = open(f'{RES_PATH}/stats_by_tissue_peakachuloop_chromhmm_{str(date.today())}.txt',"w")
...
for tis in ['ovary', 'psoas_muscle', 'heart_left_ventricle', 'lung', 'spleen', 'small_intestine', 'pancreas', 'liver', 'brain_prefrontal_cortex', 'brain_hippocampus']:
    ...
    with sns.plotting_context("paper", rc=rc):
        ...
        plt.savefig(f'{RES_PATH}/fig/{str(date.today())}/{tis}_hicQ05_anno_ecdfplot_chromhmm.{fmt}', format=fmt, dpi=400)
        plt.close()
    ### end_fig
    ...
    with sns.plotting_context("paper", rc=rc):
        ...
        plt.savefig(f'{RES_PATH}/fig/{str(date.today())}/{tis}_hicQ05_twocat_anno_ecdfplot_chromhmm.{fmt}', format=fmt, dpi=400)
        plt.close()
    ### end_fig
